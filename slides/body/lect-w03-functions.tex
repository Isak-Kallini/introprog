...
\pause
\item Med hjälp enbart funktionsnamnet får vi funktionen som ett \Alert{värde} (inga argument appliceras än):
\begin{REPLnonum}
...
\Subsection{Automatisera kompilering}
...
\begin{itemize}
  ...
  \item Då kan du använda:\\\code{scala-cli compile . --watch}\\Ändringar bevakas och kompileras om direkt.
...
\begin{itemize}
  ...
  \item Du behöver en fil som heter \code{build.sbt} som innehåller:\\\code{scalaVersion := "3.1.3"}
...
\begin{itemize}
  ...
  \item Ha ett terminalfönster öppet och kör \code{scala-cli} med \code{--watch} så att din kod kompileras vid varje sparning.
